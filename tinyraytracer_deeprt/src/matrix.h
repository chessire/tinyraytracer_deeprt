...
template <size_t DIM, typename T> struct mat {
	mat() { for (size_t i = DIM * DIM; i--; _data[i] = T()); }
	T& operator[](const size_t i) { assert(i < DIM * DIM); return _data[i]; }
	const T& operator[](const size_t i) const { assert(i < DIM * DIM); return _data[i]; }
private:
	T _data[DIM * DIM];
};
